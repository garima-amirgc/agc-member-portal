...
router.get("/current", authRequired, async (_req, res) => {
  try {
    ...
    console.error("[customer-wins] current:", e);
    return res.status(500).json({ message: "Could not load customer win." });
  }
});
...
router.get("/history", authRequired, async (_req, res) => {
  try {
    ...
    console.error("[customer-wins] history:", e);
    return res.status(500).json({ message: "Could not load customer wins." });
  }
});

router.post("/:id/move", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.CUSTOMER_WINS), async (req, res) => {
  try {
    ...
    if (code >= 500) console.error("[customer-wins] move:", e);
...
router.get("/:id", authRequired, async (req, res) => {
  try {
    ...
    console.error("[customer-wins] get:", e);
    return res.status(500).json({ message: "Could not load entry." });
  }
});

router.get("/", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.CUSTOMER_WINS), async (_req, res) => {
  try {
    const rows = await db.prepare(`SELECT * FROM ${TABLE} ORDER BY sort_order ASC, created_at DESC, id DESC`).all();
    return res.json(rows.map(shapeRow));
  } catch (e) {
    console.error("[customer-wins] list:", e);
    return res.status(500).json({ message: "Could not load entries." });
  }
});

router.post("/", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.CUSTOMER_WINS), async (req, res) => {
  try {
    ...
    console.error("[customer-wins] create:", e);
    return res.status(500).json({ message: "Could not save customer win." });
  }
});

router.put("/:id", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.CUSTOMER_WINS), async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id < 1) return res.status(400).json({ message: "Invalid id." });
    ...
    console.error("[customer-wins] update:", e);
    return res.status(500).json({ message: "Could not update entry." });
  }
});

router.delete("/:id", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.CUSTOMER_WINS), async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id < 1) return res.status(400).json({ message: "Invalid id." });
    ...
    console.error("[customer-wins] delete:", e);
