...
router.get("/current", authRequired, async (_req, res) => {
  try {
    ...
    console.error("[new-hires] current:", e);
    return res.status(500).json({ message: "Could not load new hire." });
  }
});
...
router.get("/history", authRequired, async (_req, res) => {
  try {
    ...
    console.error("[new-hires] history:", e);
    return res.status(500).json({ message: "Could not load new hires." });
  }
});
...
router.post("/:id/move", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.NEW_HIRES), async (req, res) => {
  try {
    ...
    if (code >= 500) console.error("[new-hires] move:", e);
...
router.get("/", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.NEW_HIRES), async (_req, res) => {
  try {
    ...
    console.error("[new-hires] list:", e);
...
router.post("/", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.NEW_HIRES), async (req, res) => {
  try {
    ...
    console.error("[new-hires] create:", e);
    return res.status(500).json({ message: "Could not save new hire." });
  }
});
...
router.put("/:id", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.NEW_HIRES), async (req, res) => {
  try {
    ...
    console.error("[new-hires] update:", e);
...
router.delete("/:id", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.NEW_HIRES), async (req, res) => {
  try {
    ...
    console.error("[new-hires] delete:", e);
