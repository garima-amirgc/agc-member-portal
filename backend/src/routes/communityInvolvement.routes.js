...
const TABLE = "community_involvement";
...
router.get("/current", authRequired, async (_req, res) => {
  try {
    ...
    console.error("[community-involvement] current:", e);
    return res.status(500).json({ message: "Could not load community involvement entry." });
  }
});
...
router.get("/history", authRequired, async (_req, res) => {
  try {
    ...
    console.error("[community-involvement] history:", e);
    return res.status(500).json({ message: "Could not load community involvement entries." });
  }
});

router.post("/:id/move", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.COMMUNITY_INVOLVEMENT), async (req, res) => {
  try {
    ...
    if (code >= 500) console.error("[community-involvement] move:", e);
...
router.get("/:id", authRequired, async (req, res) => {
  try {
    ...
    console.error("[community-involvement] get:", e);
    return res.status(500).json({ message: "Could not load entry." });
  }
});

router.get("/", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.COMMUNITY_INVOLVEMENT), async (_req, res) => {
  try {
    const rows = await db.prepare(`SELECT * FROM ${TABLE} ORDER BY sort_order ASC, created_at DESC, id DESC`).all();
    return res.json(rows.map(shapeRow));
  } catch (e) {
    console.error("[community-involvement] list:", e);
    return res.status(500).json({ message: "Could not load entries." });
  }
});

router.post("/", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.COMMUNITY_INVOLVEMENT), async (req, res) => {
  try {
    ...
    console.error("[community-involvement] create:", e);
    return res.status(500).json({ message: "Could not save entry." });
  }
});

router.put("/:id", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.COMMUNITY_INVOLVEMENT), async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id < 1) return res.status(400).json({ message: "Invalid id." });
    ...
    console.error("[community-involvement] update:", e);
    return res.status(500).json({ message: "Could not update entry." });
  }
});

router.delete("/:id", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.COMMUNITY_INVOLVEMENT), async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id < 1) return res.status(400).json({ message: "Invalid id." });
    ...
    console.error("[community-involvement] delete:", e);
