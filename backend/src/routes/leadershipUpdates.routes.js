...
router.post("/:id/move", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.LEADERSHIP_UPDATES), async (req, res) => {
  try {
...
router.get("/", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.LEADERSHIP_UPDATES), async (_req, res) => {
  try {
    const rows = await db.prepare(`SELECT * FROM ${TABLE} ORDER BY sort_order ASC, created_at DESC, id DESC`).all();
    return res.json(rows.map(shapeRow));
  } catch (e) {
...
router.post("/", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.LEADERSHIP_UPDATES), async (req, res) => {
  try {
...
router.put("/:id", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.LEADERSHIP_UPDATES), async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id < 1) return res.status(400).json({ message: "Invalid id." });
...
router.delete("/:id", authRequired, requireAdminGrant(ADMIN_GRANT_KEYS.LEADERSHIP_UPDATES), async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id < 1) return res.status(400).json({ message: "Invalid id." });
