...
import { badgeClassFor, fmtDateRange, fmtDays } from "./timeOffShared";
          ...
          <>
            <div className="mt-4 space-y-3">
              {(employee.balances || []).length === 0 ? (
                <p className="text-sm text-slate-500 dark:text-slate-400">No balances on file for this employee.</p>
              ) : (
                (employee.balances || []).map((b) => (
                  <div key={b.policy_code} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                    <div className={`mb-2 inline-block rounded-full px-2 py-0.5 text-xs font-medium ${badgeClassFor(b.policy_name || b.policy_code)}`}>
