...
// ─── Upload resource form (used inside modal) ─────────────────────────────────

function UploadResourceForm({ onSaved, onCancel }) {
  const [form, setForm] = useState({ category: "general", title: "", file_url: "" });
  const [uploading, setUploading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const fileRef = useRef(null);
  ...
  const handleFile = async (e) => {
    ...
    try {
      const result = await uploadResourceDocumentFile(file);
      setForm((p) => ({ ...p, file_url: result?.file_url || "" }));
    } catch (err) { setError(friendlyErrorMessage(err, "Upload failed.")); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post("/resources/documents", {
        business_unit: "AGC", category: form.category,
        title: form.title.trim(), file_url: form.file_url,
      });
      onSaved();
    } catch (err) { setError(friendlyErrorMessage(err, "Could not save.")); }
    finally { setSaving(false); }
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      {error ? <p className="text-xs text-[#E02B20]">{error}</p> : null}
      <div className="grid gap-3">
        <div>
          ...
          <input
            ...
            placeholder="e.g. Email Signatures" required
          />
        </div>
        <div>
          ...
          {form.file_url ? (
            <p className="mb-1 text-xs text-slate-500">
              ✓ Uploaded —{" "}
              <a href={resolvePublicMediaUrl(form.file_url)} target="_blank" rel="noopener noreferrer" className="text-[#0B3EAF] underline">preview</a>
              {" "}<button type="button" className="text-[#E02B20] underline" onClick={() => { setForm((p) => ({ ...p, file_url: "" })); if (fileRef.current) fileRef.current.value = ""; }}>remove</button>
            </p>
          ) : null}
          <input ref={fileRef} type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt"
            onChange={handleFile} disabled={uploading} className="text-xs text-slate-700 dark:text-slate-300" />
          {uploading && <p className="mt-1 text-xs text-slate-500">Uploading…</p>}
        </div>
      </div>
      <div className="flex gap-2">
        <button type="submit" disabled={saving || uploading} className="rounded-lg bg-[#0B3EAF] px-4 py-2 text-xs font-semibold text-white disabled:opacity-60">
...
// ─── File-type tile color ─────────────────────────────────────────────────────

function docTileColor(url) {
  const p = String(url || "").split("?")[0].toLowerCase();
  if (p.endsWith(".pdf"))                             return { bg: "#B91C1C", text: "#ffffff" };
  if (/\.(ppt|pptx)$/.test(p))                       return { bg: "#4A5568", text: "#ffffff" };
  if (/\.(doc|docx)$/.test(p))                       return { bg: "#1D4ED8", text: "#ffffff" };
  if (/\.(xls|xlsx)$/.test(p))                       return { bg: "#15803D", text: "#ffffff" };
  return { bg: "#0B3EAF", text: "#ffffff" };
}
...
export default function AgcHubPage() {
  ...
  // openModal: null | section.key | "upload"
  const [openModal, setOpenModal] = useState(null);
  const [refreshKey, setRefreshKey] = useState(0);

  // Resource docs — fetched at page level, shown as individual tiles
  ...
  const loadDocs = useCallback(async () => {
    setDocsLoading(true);
    try {
      const { data } = await api.get("/resources/facility/AGC/category/general/documents");
  ...
  const handleDeleteDoc = async (docId) => {
    ...
    await api.delete(`/resources/documents/${docId}`).catch(() => {});
    await loadDocs();
  };
  ...
  return (
    <main className={PAGE_SHELL}>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">
        ...
        {/* Individual resource doc tiles */}
        {!docsLoading && docs.map((doc) => {
          const { bg, text } = docTileColor(doc.url);
          const href = doc.url ? resolvePublicMediaUrl(doc.url) : "";
          return (
            <div key={doc.id} className="relative">
              ...
              >
                <DocIcon />
                <span className="text-sm font-bold leading-snug line-clamp-2" style={{ color: text }}>{doc.title}</span>
              </a>
              {isResourceAdmin && (
                <button
                  type="button"
                  ...
                  title="Delete"
                  className="absolute right-2 top-2 flex h-5 w-5 items-center justify-center rounded-full bg-white/20 text-[10px] text-white transition hover:bg-white/40"
                >
                  ✕
                </button>
              )}
        ...
        {/* Admin: add resource tile */}
        {isResourceAdmin && !docsLoading && (
          <button
            type="button"
            onClick={() => setOpenModal("upload")}
            className="flex items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-slate-300 px-4 py-4 text-slate-500 transition hover:border-[#0B3EAF] hover:text-[#0B3EAF] dark:border-slate-600 dark:text-slate-400"
          >
            <span className="text-xl leading-none">+</span>
            <span className="text-sm font-semibold">Add resource</span>
          </button>
        )}
      </div>
