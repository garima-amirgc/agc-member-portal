...
export const ENGAGEMENT_ART_KINDS = [
  "fireworks",
  "ribbon",
  "women",
  "earth",
...
export const DEFAULT_CALENDAR_MONTHS = [
  {
    name: "January",
...
export function normalizeEngagementMonths(monthsIn) {
  ...
  for (let i = 0; i < 12; i += 1) {
    const m = monthsIn[i];
    const d = DEFAULT_CALENDAR_MONTHS[i];
    const name =
    ...
    const art =
      m && typeof m.art === "string" && ENGAGEMENT_ART_KINDS.includes(m.art) ? m.art : d.art;
